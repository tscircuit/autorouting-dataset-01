/** Randomly generated circuit 224. */
export default () => (
  <board width="32.00mm" height="19.00mm">
    <diode name="diode-1" footprint="SOD-123" pcbX={0.00} pcbY={0.00} connections={{
        pin2: "net.N1",
        pin1: "net.N4",
      }} />
    <diode name="diode-2" footprint="SOD-123" pcbX={-9.00} pcbY={-4.50} connections={{
        pin1: "net.N3",
        pin2: "net.N4",
      }} />
    <capacitor name="capacitor-1" footprint="0603" pcbX={-6.00} pcbY={6.00} connections={{
        pin2: "net.N2",
        pin1: "net.N2",
      }} capacitance="0.1uF" />
    <resistor name="resistor-1" footprint="0402" pcbX={-3.00} pcbY={-5.00} connections={{
        pin2: "net.N1",
        pin1: "net.N6",
      }} resistance="1k" />
    <diode name="diode-3" footprint="SOD-123" pcbX={-1.50} pcbY={3.50} connections={{
        pin2: "net.N4",
        pin1: "net.N5",
      }} />
    <chip name="chip-1" footprint="pinrow6" pcbX={5.00} pcbY={-5.00} connections={{
        pin2: "net.N3",
        pin1: "net.N3",
        pin3: "net.N3",
        pin6: "net.N4",
        pin4: "net.N6",
        pin5: "net.N6",
      }} manufacturerPartNumber="GENERIC" />
    <transistor name="transistor-1" footprint="SOT-23" pcbX={7.00} pcbY={1.00} connections={{
        pin3: "net.N1",
        pin2: "net.N5",
        pin1: "net.N5",
      }} type="npn" />
    <resistor name="resistor-3" footprint="0603" pcbX={-8.00} pcbY={1.00} connections={{
        pin1: "net.N6",
        pin2: "net.N6",
      }} resistance="1k" />
  </board>
)
