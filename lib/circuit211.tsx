/** Randomly generated circuit 211. */
export default () => (
  <board width="36.00mm" height="36.00mm">
    <resistor name="resistor-1" footprint="0603" pcbX={0.00} pcbY={0.00} connections={{
        pin2: "net.N2",
        pin1: "net.N8",
      }} resistance="1k" />
    <capacitor name="capacitor-1" footprint="0603" pcbX={-5.00} pcbY={-5.00} connections={{
        pin2: "net.N9",
        pin1: "net.N15",
      }} capacitance="0.1uF" />
    <inductor name="inductor-1" footprint="0402" pcbX={-3.00} pcbY={3.00} connections={{
        pin1: "net.N3",
        pin2: "net.N8",
      }} inductance="10uH" />
    <transistor name="transistor-1" footprint="SOT-23" pcbX={2.00} pcbY={-6.00} connections={{
        pin1: "net.N3",
        pin3: "net.N4",
        pin2: "net.N11",
      }} type="npn" />
    <transistor name="transistor-2" footprint="SOT-23" pcbX={6.00} pcbY={7.50} connections={{
        pin3: "net.N5",
        pin2: "net.N11",
        pin1: "net.N13",
      }} type="npn" />
    <inductor name="inductor-2" footprint="0603" pcbX={-10.50} pcbY={3.00} connections={{
        pin2: "net.N3",
        pin1: "net.N6",
      }} inductance="10uH" />
    <diode name="diode-1" footprint="SOD-123" pcbX={7.00} pcbY={0.00} connections={{
        pin2: "net.N10",
        pin1: "net.N12",
      }} />
    <transistor name="transistor-3" footprint="SOT-23" pcbX={1.50} pcbY={3.50} connections={{
        pin1: "net.N8",
        pin2: "net.N14",
        pin3: "net.N14",
      }} type="npn" />
    <transistor name="transistor-4" footprint="SOT-23" pcbX={-9.00} pcbY={9.00} connections={{
        pin3: "net.N4",
        pin1: "net.N11",
        pin2: "net.N14",
      }} type="npn" />
    <resistor name="resistor-2" footprint="0603" pcbX={-4.00} pcbY={-2.00} connections={{
        pin2: "net.N7",
        pin1: "net.N7",
      }} resistance="1k" />
    <inductor name="inductor-3" footprint="0402" pcbX={-2.00} pcbY={9.00} connections={{
        pin2: "net.N9",
        pin1: "net.N12",
      }} inductance="10uH" />
    <transistor name="transistor-5" footprint="SOT-23" pcbX={9.00} pcbY={-9.00} connections={{
        pin2: "net.N1",
        pin3: "net.N9",
        pin1: "net.N13",
      }} type="npn" />
    <chip name="chip-1" footprint="soic16" pcbX={-9.50} pcbY={-9.50} connections={{
        pin10: "net.N1",
        pin16: "net.N1",
        pin1: "net.N1",
        pin3: "net.N2",
        pin8: "net.N3",
        pin15: "net.N6",
        pin14: "net.N6",
        pin5: "net.N6",
        pin12: "net.N7",
        pin7: "net.N7",
        pin13: "net.N10",
        pin9: "net.N10",
        pin6: "net.N10",
        pin2: "net.N11",
        pin11: "net.N13",
        pin4: "net.N13",
      }} manufacturerPartNumber="GENERIC" />
    <diode name="diode-2" footprint="SOD-123" pcbX={1.00} pcbY={-12.00} connections={{
        pin1: "net.N5",
        pin2: "net.N15",
      }} />
  </board>
)
